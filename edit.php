...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
  ...
  <!-- 
	...
	<div class="box-wrap">
		...
		<?php 
			$id = $_GET['id'];
			$sql = mysql_query("SELECT * FROM kegiatan WHERE id_kegiatan='$id'");
					...
					<form method="POST">
						<div class="form-group row">
							<div class="col-md-6 field">
								...
								<input type="text" name="tgl" id="firstname" value="<?php echo $data['tanggal']; ?>" class="form-control" disabled>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-md-12 field">
								<label for="email">Judul</label>
								<input type="text" name="judul" id="email" value="<?php echo $data['judul']; ?>" class="form-control">
							</div>
						</div>
						<div class="form-group row">
							<div class="col-md-12 field">
								<label for="message">Isi</label>
								<textarea name="isi" id="message" cols="30" rows="10" class="form-control">
									<?php echo $data['isi']; ?>
								</textarea>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-md-12 field">
								<input type="submit" name="post" id="submit" class="btn btn-primary" value="Edit">
...
<?php 
	if (isset($_POST['post'])) {
		...
		$query="UPDATE kegiatan SET judul='$judul', isi='$isi'";
		$hasil=mysql_query($query);

		if ($hasil) {
			echo "<script>alert('Berhasil');window.location='kegiatan.php'</script>";
		} else {
			echo "<script>alert('gagal');window.location='edit.php'</script>";
		}
